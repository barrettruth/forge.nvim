...
describe('git sections', function()
  ...
  it('lists local branches and supports switch and browse actions', function()
    ...
    assert.same({ '* ', 'ForgeBranchCurrent' }, captured.picker.entries[1].display[1])
  ...
  it(
    'falls back to the local branch when the upstream log fetch fails and trims commit fields',
    function()
      local ctx = {
        forge = {
          browse_commit = function(_, sha)
      ...
      vim.system = function(cmd, _, cb)
        ...
        if key == 'git for-each-ref --format=%(upstream:short) refs/heads/main' then
          result.stdout = 'origin/main\n'
        elseif
          key == 'git log --max-count=101 --format=%H%x1f%h%x1f%s%x1f%an%x1f%ct%x1e origin/main'
        then
          result.code = 128
