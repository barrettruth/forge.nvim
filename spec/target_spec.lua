...
describe('target parsing', function()
  ...
  it('parses plain browse revisions without repo syntax', function()
    local target = require('forge.target')
    local branch = assert(target.parse_browse_rev('main'))
    local sha = assert(target.parse_browse_rev('deadbee'))

    assert.equals('main', branch.rev)
    assert.equals('deadbee', sha.rev)
  end)
  ...
  it('rejects unresolved symbolic and malformed addresses', function()
    ...
    local _, invalid_browse_rev = target.parse_browse_rev('@main')
    ...
    assert.equals('invalid revision: @main', invalid_browse_rev)
