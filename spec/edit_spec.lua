...
describe('editing an item', function()
  ...
  it('opens beside the item, keeping its window', function()
    local var = showing()
    local item = vim.api.nvim_get_current_win()
    edit.open(var)

    assert.are_not.equal(item, vim.api.nvim_get_current_win())
    assert.is_true(vim.api.nvim_win_is_valid(item))
