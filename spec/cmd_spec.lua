...
describe('command schema', function()
  ...
  it(
    'attaches implicit closed-PR reopen disambiguation modifiers to normalized commands',
    function()
      local reopen = assert(cmd.parse({ 'pr', 'reopen', 'repo=upstream', 'head=origin@topic' }))

      assert.equals('reopen', reopen.name)
      assert.same({}, reopen.subjects)
      assert.equals('owner/upstream', reopen.parsed_modifiers.repo.slug)
      assert.equals('topic', reopen.parsed_modifiers.head.rev)
      assert.equals('owner/current', reopen.parsed_modifiers.head.repo.slug)
    end
  )
  ...
  it('exposes per-verb modifiers for canonical operations', function()
    ...
    assert.same({ 'repo', 'head' }, cmd.modifier_names('pr', 'reopen'))
  ...
  it('rejects non-numeric subjects on PR-numbered verbs', function()
    ...
    local _, pr_reopen_err = cmd.parse({ 'pr', 'reopen', 'topic' })
    ...
    assert.equals('invalid PR number: topic', pr_reopen_err.message)
  ...
  it('accepts purely numeric subjects on PR/issue verbs', function()
    ...
    assert.is_not_nil(cmd.parse({ 'pr', 'reopen', '42' }))
