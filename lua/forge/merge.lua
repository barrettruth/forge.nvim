...
--- github's enum spells a merge commit "MERGE", which reads badly on its own.
...
local function write(lines, buf, u, var, method, auto)
  ...
  be.write({
    ...
    -- Into the window the message was written in. Nothing is left to close.
    -- 'bufhidden' is "wipe": the item taking the window takes the message
    -- buffer away with it.
...
function M.open(var, method, auto)
  ...
  compose.open({
    -- Named for the method and for whether it waits. Picking one after
    -- another must not hand back the message written for the first.
    ...
    label = var.label,
    tag = var.tag,
    -- A bypass goes past a rule, and is drawn as an error. github does not
    -- honour a bypass when a waiting merge comes due. The two words never
    -- appear together.
