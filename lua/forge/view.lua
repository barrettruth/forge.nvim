...
---
--- What the buffer shows, on the same terms as "gX". The register is whichever
--- one the mapping was given, so `"+gy` reaches the clipboard and 'clipboard'
--- is honoured without forge reading it: |v:register| answers for both.
function M.yank()
  local u = M.current()
  if not u then
    log.warn('no url for this buffer')
